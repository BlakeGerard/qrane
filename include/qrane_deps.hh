...
namespace qrane {
	...
	class DependenceProfile {
		...
		private:
			...
			void construct_dependence_graph(const std::map<qop_id, std::shared_ptr<Qop>>& qops,
											const qrane_options* opt);
			void add_dependence(const qop_id accessor, const qubit_id accessed, const access_type_e type);
		...
		public:
			...
			std::array<unsigned int, 2> lookahead_breadth(unsigned int breadth, 
				std::vector<qop_id>& frontier, std::map<qop_id, std::shared_ptr<Qop>>& qops);
			std::array<unsigned int, 2> lookahead_depth(unsigned int depth,
				std::vector<qop_id>& frontier, std::map<qop_id, std::shared_ptr<Qop>>& qops);
				
			// isl functionality
			isl_union_map* build_implicit_schedule(isl_ctx* ctx, const membership_map_t& membership);
			
			// Printing and string formatting
			std::string get_networkx_edge_list_str();
			std::string get_gates_networkx_edge_list_str(const std::map<qop_id, std::shared_ptr<Qop>>& qops);
