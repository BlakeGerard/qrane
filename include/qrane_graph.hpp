...
*/
template <typename T>
adjacency_t<T> Graph<T>::get_vertex_set() const {
...
*/
template <typename T>
adjacency_t<T> Graph<T>::get_neighbors(T v) const {
	if (G_.contains_vertex(v)) {
		return G_[v];
	}
...
*/
template <typename T>
unsigned int Graph<T>::num_vertices() const {
    return G_.size();
}
...
*/
template <typename T>
unsigned int Graph<T>::num_edges() const {
    unsigned int cnt = 0;
    for (const auto& entry : G_) {
...
template <typename T>
bool Graph<T>::contains_edge(T v, T u) const {
	if (!G_.contains_vertex(v)) {
		return false;
	}
	if (!G_.contains_vertex(u)) {
...
*/
template <typename T>
std::vector<T> Graph<T>::topological_ordering() const {
    Graph H = G_;
...
*/
template <typename T>
std::string Graph<T>::in_degrees_to_string() const {
    std::ostringstream strm;
    strm << "{" << std::endl;
    for (const auto& entry : in_degree_) {
...
*/
template <typename T>
std::string Graph<T>::to_python_tuple_list() const {
    std::ostringstream strm;
...
/*
	Construct a qrane::Graph<int> from a Python-style list of two-tuples of integers.
