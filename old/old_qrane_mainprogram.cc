...
// #define STMT_REGEX "S[0-9]+\\((.+)\\);"   // S34(c0 + 2);
// #define STMT_NAME_REGEX "S[0-9]+"         // S34
// #define ROUNDS 2
...
// 	if (!QUIET_MODE) { std::cout << "Converting statements to qrane tuples ... "; }
// 	convert_stmtlist_to_vector_form();
// 	if (!QUIET_MODE) { std::cout << " Done.\n";}

// 	if (!QUIET_MODE) { std::cout << "Constructing domains of one dimension ... "; }
// 	if (ONED_MODE == DDG) { process_ddg(); }
// 	else { process_circuit(); }
// 	if (!QUIET_MODE) { std::cout << " Done.\n";}

// 	if (!QUIET_MODE) { std::cout << "Attempting iterative merging of lower dimension domains to higher dimension domains ... "; }
// 	combine_domains();
// 	if (!QUIET_MODE) { std::cout << " Done.\n"; }
...
// 	if (!QUIET_MODE) { std::cout << "Building circuit form ... " << std::flush; }
...
// 		//chosen_path = look_ahead_policy(frontier, LOOKAHEAD_DEPTH);
...
// 			if (remaining < SEARCH_LIMIT) { lim_it = std::next(i, remaining); } 
// 			else { lim_it = std::next(i, SEARCH_LIMIT); }

// 			// For SEARCH_LIMIT other domains in the set
// 			for (domain_map_t::iterator j = std::next(i, 1); j != std::end(this->domain_map) && j != lim_it; ++j) {

// 				unsigned int limit;
// 				if (SEARCH_LIMIT == 0) { limit = SEARCH_LIMIT + 1; } 
// 				else { limit = 0; }
// 				for (auto& pair : my_combinations) {
// 					if (limit == SEARCH_LIMIT) { break; }
...
//             //if (SEARCH_LIMIT > 0) { std::shuffle(all_combinations.begin(), all_combinations.end(), std::default_random_engine()); }
...
// 			domain_map_t::iterator lim_it;
// 			auto remaining = std::distance(i, std::end(this->domain_map));
// 			if (remaining < SEARCH_LIMIT) {
// 				lim_it = std::next(i, remaining);
// 			} else {
// 				lim_it = std::next(i, SEARCH_LIMIT);
// 			}
...
//     none
// */
// void qrane_mainprogram::build_isl_domain_read_write_schedule(SCHEDULE_ALGORITHM SCHEDULE_MODE) {
...
//  	if (!QUIET_MODE) { std::cout << "Scheduling ... " << std::flush; }
// 	scop->schedule = initialize_recovered_schedule(SCHEDULE_MODE);
// 	scop->schedule = isl_union_map_intersect_domain(scop->schedule, isl_union_set_copy(scop->domain));
//     if (!QUIET_MODE) { std::cout << "Done.\n" << std::flush; }
//   }
// };
...
// __isl_give isl_union_map* qrane_mainprogram::initialize_recovered_schedule(SCHEDULE_ALGORITHM SCHEDULE_MODE) {
...
//   if (CHECK_MODE || SCHEDULE_MODE != DEFAULT) {
...
//   if (CHECK_MODE > 0) {
//     //std::cout << "DefaultSchedule := " << isl_union_map_to_str(isl_union_map_copy(default_schedule)) << std::endl << std::endl;
//   }
// };
...
// 	assert(isl_options_set_schedule_max_constant_term(ctx, 1) == isl_stat_ok);
// 	assert(isl_options_set_schedule_max_coefficient(ctx, 10) == isl_stat_ok);
...
// 		if (!QUIET_MODE) { std::cout << "Feautrier chosen.\n"; }
//         break;
//     } 
...
// 	if (!QUIET_MODE) { std::cout << "computing schedule ..." << std::flush; }
//     isl_schedule* sched = isl_schedule_constraints_compute_schedule(cons);
// 	if (!QUIET_MODE) { std::cout << "Done.\n" << std::flush; }

// 	this->scop->schedule = isl_schedule_get_map(sched);
// };
