...
typedef std::unordered_map<std::vector<int>,
                           std::pair<lex_type, std::deque<unsigned int>>,
                           VectorOfIntsHasher>
    access_graph;

enum frame_element { ID = 0, STRIDE, LEX_TYPE, VISITED, PATH };
typedef std::stack<std::tuple<unsigned int, int, lex_type,
...
private:
  ...
  qrane_deps deps;
  std::string codegen_c_str; // String to store codegen output with ISL_FORMAT_C
                             // flag enabled
  ...
  plinko_grid circuit;
  std::unordered_map<std::string, std::vector<qrane_domain>>
      gate_domain_map; // Group domains by gate type
  ...
  __isl_give isl_set *
  build_ND_domain_from_sets(std::vector<qrane_domain> &mergees);
  __isl_give isl_mat *compute_access_relations(isl_mat *mat);
  __isl_give isl_mat *
  concat_matrices_dimwise(std::vector<qrane_domain> &mergees);
  void attempt_merge(std::vector<qrane_domain> &resultant,
                     std::vector<qrane_domain> domains);
  __isl_keep bool check_consistency(isl_mat *mat, std::size_t dim,
                                    std::size_t args);
  ...
  std::vector<unsigned int>
  get_ones_from_level(std::vector<unsigned int> &level);
  std::vector<unsigned int>
  get_twos_from_level(std::vector<unsigned int> &level);
  std::vector<unsigned int>
  find_longest_ones_path(std::vector<unsigned int> &ones,
                         std::size_t qreg_size);
  std::vector<unsigned int>
  find_longest_twos_path(std::vector<unsigned int> &twos,
                         std::size_t qreg_size);
  std::vector<unsigned int>
  get_most_frequent_gate_type(const std::vector<unsigned int> &level);
  std::vector<unsigned int>
  one_qubit_clearing_policy(std::vector<unsigned int> &level);
  std::vector<unsigned int> look_ahead_policy(std::vector<unsigned int> &level,
                                              unsigned int depth);
  std::array<unsigned int, 2>
  dependence_graph_lookahead(std::vector<unsigned int> &path,
                             unsigned int depth);
  ...
  stride_graph generate_stride_graph(const std::vector<unsigned int> &level,
                                     unsigned int qreg_size, unsigned int args);
  ray_graph generate_ray_graph(const std::vector<unsigned int> &level,
                               unsigned int qreg_size);
  std::vector<unsigned int> get_next_grid_row(plinko_grid &P,
                                              std::vector<bool> &seen,
                                              unsigned int qreg_size);
  std::vector<unsigned int>
  longest_path_search(stride_graph &G, std::vector<unsigned int> ordering);
  std::vector<unsigned int>
  longest_valid_path_search(stride_graph G, std::vector<unsigned int> ordering);
  std::vector<unsigned int>
  get_longest_valid_path(std::vector<std::vector<unsigned int>> paths);
  std::array<unsigned int, 2> look_ahead(plinko_grid circuit,
                                         unsigned int depth);
  ...
  domain_map_t greedy_domain_selection(access_graph &G,
                                       unsigned int current_dim);
  qrane_domain merge_domains(const std::vector<unsigned int> &domains,
                             lex_type lex, unsigned int current_dim);
  __isl_give isl_set *
  combine_isl_local_domains(const std::vector<unsigned int> &domains,
                            unsigned int current_dim);
  __isl_give isl_mat *
  combine_access_matrices(const std::vector<unsigned int> &domains,
                          unsigned int current_dim);
  bool inconsistent_or_rational_matrix(const std::vector<unsigned int> &domains,
                                       unsigned int current_dim);
  bool extended_domain_inconsistent(unsigned int new_dom,
                                    std::vector<unsigned int> path,
                                    unsigned int current_dim);
  bool extended_domain_inconsistent(unsigned int lhs, unsigned int rhs,
                                    unsigned int current_dim);
  domain_map_t greedy_domain_selection(candidate_list candidates,
                                       unsigned int current_dim);
  ...
  qop_identifier generate_qop_identifier(qrane_qop *qop,
                                         std::size_t qop_num_type);
  void sort_favor_linearity(qasm_ops_list &ops_list);
  bool dependence_constrained_lex_order(const qop_identifier &a,
                                        const qop_identifier &b);
...
public:
  ...
  std::string
  print_reconstruction_histogram(std::vector<qrane_domain> dom_list);
};
