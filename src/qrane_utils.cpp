...
std::vector<qop_id> stride_graph_long_path_search(
	...
	if (one_qubit_qops.empty()) {
		return stride_graph_longest_path_of_constant_weight(two_qubit_qops, all_qops);
	} else {
		return stride_graph_longest_path_of_constant_weight(one_qubit_qops, all_qops);
	}
}
...
std::vector<qop_id> get_qops_of_most_common_gate_type(
	...
	auto max = std::max_element(gate_types.begin(), gate_types.end(), 
...
// 			const std::size_t m_size = m.size();
// 			std::size_t i = 0;
// 			std::array<A, m_size> tmp;
...
// 			std::vector<std::shared_ptr<Element>> ret = std::vector<std::shared_ptr<Element>>();
...
// 		std::vector<std::vector<std::shared_ptr<Element>>> split_into_n_components(
...
// 			auto vec = std::vector<std::vector<std::shared_ptr<Element>>>(size);
...
// 				std::vector<std::shared_ptr<Element>>::iterator start_itr = std::next(stmts.begin(), k*chunks);
// 				std::vector<std::shared_ptr<Element>>::iterator end_itr = std::next(stmts.begin(), k*chunks + chunks);
...
// 			isl_mat* coeff_mat = get_zero_matrix(ctx, coeff_rows, coeff_cols); 
// 			isl_val* wrk1;
// 			isl_val* wrk2;
// 			isl_val* wrk3;
