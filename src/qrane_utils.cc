...
std::vector<std::vector<qrane_statement*>> qrane_utils::split_into_n_components(
	std::vector<qrane_statement*> stmts, unsigned int chunks) {
...
std::vector<unsigned int> qrane_utils::adj_list_topological_ordering(stride_graph G) {
...
void qrane_utils::adj_list_topological_ordering_util(unsigned int start, stride_graph& G, std::unordered_map<unsigned int, bool>& visited, std::stack<unsigned int>& ordering) {
	visited[start] = true;
...
void qrane_utils::plinko_grid_print(plinko_grid& P, qasm_ops_list& qops_list) {
	...
	for (auto queue : P) {
		...
		while (!queue.empty()) {
			...
			if (std::get<ARGS>(qops_list[queue.front()]).size() == 2) { std::cout << "*"; }
			std::cout << " ";
			queue.pop_front();
		}
		std::cout << "]\n";
...
*/
isl_mat* qrane_utils::back_substitute_isl_mat(isl_ctx* ctx, __isl_keep isl_mat* mat, unsigned int args, unsigned int dim) {
