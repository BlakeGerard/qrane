...
Program::Program(isl_ctx* ctx, const qrane_options* opt, program_id id) {
  ...
  id_ = id;
  num_qops_ = 0;
  full_scop_ = new full_scop();
  statements_ = std::map<statement_id, Statement>();
};

Program::~Program() { delete full_scop_; }

/*
        Populate the Program with a list of OpenQASM elements
        for it to analyze.
*/
void Program::initialize(
    std::vector<std::shared_ptr<qrane::Element>> elements) {
  elements_ = elements;
};

/*
        Drive the delinearization process over the given OpenQASM elements.
*/
void Program::delinearize() {
  if (!opt_->quiet) {
    std::cout << "Extracting qops from OpenQASM program elements ... ";
  }
  extract_qops();
  if (!opt_->quiet) {
  ...
  if (!opt_->quiet) {
    std::cout << "Reconstructing 1D Scops ... ";
  }
  one_dimensional_scop_reconstruction(default_one_dimensional_selection_policy);
  if (!opt_->quiet) {
    std::cout << " Done" << std::endl;
  }

  if (!opt_->quiet) {
    std::cout << "Reconstructing ND Scops ... ";
  }
  n_dimensional_scop_reconstruction(default_n_dimensional_selection_policy);
  if (!opt_->quiet) {
    std::cout << " Done" << std::endl;
  }

  if (!opt_->quiet) {
    std::cout << "Building ISL output_scop ... ";
  }
  build_output_scop();
  if (!opt_->quiet) {
    std::cout << " Done";
...
/*
        Get the Program id
*/
program_id Program::id() const { return id_; };

/*
        Get num qops processed by this Program
*/
std::size_t Program::num_qops() const { return num_qops_; };

/*
        Get number of OpenQASM elements for this program to analyze.
*/
std::size_t Program::num_elements() const { return elements_.size(); };

/*
        Get number of statements reconstructed by this Program.
*/
std::size_t Program::num_statements() const { return statements_.size(); };

/*
  Get the output_scop of ISL structures for the program.
*/
output_scop Program::get_output_scop() const { return output_scop_; };
...
/*
  ...
  possibly reordered.
*/
std::string Program::codegen() const { return std::string(); }
...
void Program::extract_qops() {
  for (const auto& elem : elements_) {
    if (elem->is_qop()) {
      ...
      qops_.insert(std::make_pair(new_id, qop));
    }
  }
  num_qops_ = qops_.size();
};

void Program::dependence_analysis() {
  dependence_profile_ = DependenceProfile(qops_, opt_);
};

/*
        Drive the 1D reconstruction phase.
*/
void Program::one_dimensional_scop_reconstruction(
    std::function<std::vector<qop_id>()> selection_policy) {
  // Counter of processed qops
  std::unordered_set<qop_id> qops_processed;

  // Create new 1D statements until all qops are processed
  while (qops_processed.size() != num_qops_) {
    // selection_policy gives a set of qops to group
    std::vector<qop_id> group = selection_policy();

    // Verify the selection_policy gave us all new qops
    for (const auto id : group) {
      if (!qops_processed.insert(id).second) {
        throw OnedSelectionPolicyException(
            R"("A qop returned by the provided selection_policy
                was previously processed. Adjust the policy to
                return either a unique list of qop_ids or an
                empty list at each call.")");
      }
    }

    // Create the new domain and store it in the domain_map
    create_and_store_new_statement(group);
  }
};

std::vector<qop_id> Program::default_one_dimensional_selection_policy() {
  // Fetch the frontier of the dependence graph
  auto frontier = dependence_profile_.get_dependence_frontier();

  // Use stride graph search approach
  auto frontier_subset = Utils::stride_graph_long_path_search(frontier, qops_);

  // Pop the qops from the dependence graph
  dependence_profile_.remove_frontier_subset(frontier_subset);

  // Frontier subset chosen as the next domain
  return frontier_subset;
};
...
void Program::build_output_scop() {
  ...
  for (const auto& stmt : statements_) {
    ...
    output_scop_.read =
        isl_union_map_add(output_scop_.read, stmt.get_read_relations());
    output_scop_.write =
        isl_union_map_add(output_scop_.write, stmt.get_write_relations());
    output_scop_.schedule =
        isl_union_map_add(output_scop_.schedule, stmt.get_implicit_schedule());
    output_scop_.arguments =
        isl_union_map_add(output_scop_.arguments, stmt.get_arguments_map());
  }
  return ret;
}
...
// std::string Program::get_membership_map_reverse_str() {
// 	return this->deps.get_membership_map_reverse_str();
// };

// unsigned long long Program::get_num_points() {
// 	assert(!scop->final_domain_list.empty());
// 	if (num_points == 0) { compute_num_points(); }
// 	return num_points;
// }

// void Program::compute_num_points() {
// 	assert(!scop->final_domain_list.empty());
...
// void Program::increment_num_qops() {
// 	this->num_qops += 1;
// }

// qrane_graph<qop_id> Program::get_dependence_graph() {
// 	return this->deps.get_dependence_graph();
// };

// void Program::set_substrs(std::vector<qrane_statementlist> substrs) {
// 	this->substrs = substrs;
// }

// void Program::set_num_qops(unsigned int num_qops) {
// 	this->num_qops = num_qops;
// }

// unsigned long long Program::get_num_qubit_exprs() {
// 	return this->num_qubit_exprs;
// }

// bool Program::check_all_qops_accounted() {
// 	compute_num_points();
// 	std::cout << "num_qops: " << this->num_qops << ", " << "num_points: " <<
// this->num_points << std::endl; 	return this->num_qops ==
// this->num_points;
// };

// unsigned int Program::get_time_min() {
...
// unsigned int Program::get_time_max() {
// 	membership_map membership = deps.get_membership();
...
// t_qrane_scop* Program::get_scop() {
// 	return this->scop;
// }

// bool vec_size_compare(const std::vector<qop_id>&a, const
// std::vector<qop_id>&b) { 	return a.size() < b.size();
// };

// bool id_vec_size_compare(const std::pair<std::string, std::vector<qop_id>>&a,
// 						 const std::pair<std::string,
// std::vector<qop_id>>&b) { 	return a.second.size() < b.second.size();
// };

// bool rg_compare(const std::pair<std::pair<qop_id, float>,
// std::deque<qop_id>>&a, 				const
// std::pair<std::pair<qop_id, float>, std::deque<qop_id>>&b) { 	return
// a.second.size() < b.second.size();
// }

// void Program::set_scop(t_qrane_scop* new_scop) {
// 	this->scop = new_scop;
// 	this->deps.membership = scop->membership;
// }

// std::vector<qop_id> Program::get_ones_from_level(std::vector<qop_id>& level)
// { 	std::vector<qop_id> ones; 	for (std::size_t i = 0; i <
// level.size(); ++i) { 		if (qops[level[i]]->is_1Q_gate()) {
...
// std::vector<qop_id> Program::get_twos_from_level(std::vector<qop_id>& level)
// { 	std::vector<qop_id> twos; 	for (std::size_t i = 0; i <
// level.size(); ++i) { 		if (qops[level[i]]->is_2Q_gate()) {
...
// void Program::create_fresh_domain(isl_ctx* ctx, std::vector<qop_id>& path) {
// 	// Construct the domain object
// 	std::string gate_id = qops[path[0]]->get_operation();
// 	unsigned int num_args = qops[path[0]]->num_args();
// 	qrane_domain dom = qrane_domain(ctx, num_domains, gate_id, num_args,
// qops, path); 	this->domain_map[num_domains] = dom;
...
// qrane_grid Program::generate_circuit_grid() {
...
// std::vector<qop_id> Program::get_next_grid_row(qrane_grid& P,
// std::unordered_set<qop_id>& grouped) {

// 	// First pop any ids that have already been grouped
// 	for (auto& lane : P) {
// 		while (!lane.second.empty() &&
// grouped.count(lane.second.front())) { lane.second.pop();
// 		}
// 	}
...
// std::vector<qop_id> Program::one_qubit_clearing_policy(std::vector<qop_id>&
// level) { 	std::vector<qop_id> ones = get_ones_from_level(level);
...
// std::vector<qop_id> Program::look_ahead_policy(std::vector<qop_id>& level,
// unsigned int depth) { 	std::vector<qop_id> ones =
// get_ones_from_level(level); 	std::vector<qop_id> twos =
// get_twos_from_level(level); 	if (depth == 0) { 		if (ones.size()
// > twos.size()) { 			return find_longest_ones_path(ones,
// qreg_size); 		} else if (ones.size() < twos.size()) {
// return find_longest_twos_path(twos, qreg_size); 		} else {
// return find_longest_ones_path(twos, qreg_size);
// 		}
// 	}
// 	std::array<unsigned int, 2> forecast =
// deps.dependence_graph_lookahead_breadth(level, depth, num_qops, qops);
// if
// (twos.empty()) { 		return find_longest_ones_path(ones, qreg_size);
// } else if (ones.empty()) { 		return find_longest_twos_path(twos,
// qreg_size); 	} else { 		if (forecast[0] < forecast[1]) {
// return find_longest_ones_path(ones, qreg_size); 		} else if
// (forecast[0] > forecast[1]) { 			return
// find_longest_twos_path(twos, qreg_size); 		} else {
// return find_longest_ones_path(ones, qreg_size);
// 		}
// 	}
// };
...
// //
// std::vector<qop_id> Program::find_longest_ones_path(
// 	std::vector<qop_id>& ones, std::size_t qreg_size) {

// 	// Get the points of the most frequently occurring gate type on this
// level 	std::vector<qop_id> freq_gate_points =
...
// 	std::vector<qop_id> ordering =
// qrane_utils::adj_list_topological_ordering(sg); 	std::vector<qop_id>
// longest_path = longest_valid_path_search(sg, ordering);

// 	return longest_path;
// };

// std::vector<qop_id> Program::find_longest_twos_path(
...
// 		auto max_it = std::max_element(rg.begin(), rg.end(),
// rg_compare);

// 		// Convert the deque to a vector
// 		auto level = std::vector<qop_id>();
...
// 		std::vector<qop_id> selected_path = longest_valid_path_search(G,
// ordering);

// 		//if (!selected_path.empty()) {
// gate_type_paths.push_back(selected_path); break;}
// 	//}
// 	// Return the largest path found of all gate_types
// 	return selected_path;
// };

// std::vector<qop_id> Program::get_most_frequent_gate_type(const
// std::vector<qop_id>& level) { 	std::unordered_map<std::string,
// std::vector<qop_id>> separated_points; 	for (const auto point : level) {
// 		std::string gate_type = qops[point]->get_operation();
// 		separated_points[gate_type].push_back(point);
// 	}
// 	return std::max_element(separated_points.begin(),
// separated_points.end(), id_vec_size_compare)->second;
// };

// /*

// */
// stride_graph Program::generate_stride_graph(const std::vector<qop_id>&
// qop_ids, unsigned int qreg_size, unsigned int args) { 	stride_graph G;
// 	G.reserve(qreg_size);
...
// 		if (args_1.at(1)->get_index() - args_0.at(1)->get_index() != 0)
// { index = 1; } 		else if (args_1.at(0)->get_index() -
// args_0.at(0)->get_index()
// != 0) { index = 0; } 		else { index = 0; } 	} else {
// index = 0;
// 	}
...
// 			// Create the neighbor as (global_id, [strides]) and
// push to j's adjacency list 			if (stride > 0) {
...
// std::vector<qop_id> Program::longest_valid_path_search(stride_graph G,
// std::vector<qop_id> ordering) { 	std::vector<std::vector<qop_id>> paths;

// 	// While we can still pick a source vertex
// 	while (!ordering.empty()) {

// 		// Grab the first zero in-degree vertex in the topological
// ordering 		qop_id src = ordering.front();

// 		// Initialize local stack for perform DFS from src and
// longest_path variable 		frame_stack stack = frame_stack();
...
// std::vector<qop_id>()); 		stack.push(start_frame);
...
// 			// Put the current global id into the visited and path
// vectors 			if (!std::get<VISITED>(curr_frame)[curr_id]) {
...
// 			for (auto i = G[curr_id].begin(); i != G[curr_id].end();
// ++i)
// {

// 				// If the vertex is unvisited, we have two
// possibilities 				if
// (!std::get<VISITED>(curr_frame)[(*i).first]) {

// 					// If we are at the first branch from
// the search, then we have yet to establish a stride
// 					// Compute the stride and lexicographic
// order, then build the new frame 					if
// (std::get<STRIDE>(curr_frame) == 0) {
// int time_space_diff = i->first - std::get<PATH>(curr_frame).back();
// lex_type lex; 						if
// (time_space_diff > 0) { lex = LEX_POS; } else if (time_space_diff < 0) { lex
// = LEX_NEG; } // ADDED CONTINUE auto new_frame = std::make_tuple(i->first,
// i->second, lex,
// std::get<VISITED>(curr_frame), std::get<PATH>(curr_frame));
// stack.push(new_frame); frame_pushed = true;

// 					// Otherwise, we only push a new frame
// when the path stride (weight) equals the established stride } else { int
// time_space_diff = i->first - std::get<PATH>(curr_frame).back(); lex_type lex;
// if (time_space_diff > 0)
// { lex = LEX_POS; } 						else if
// (time_space_diff < 0) { lex = LEX_NEG; } // ADDED CONTINUE

// 						if (i->second ==
// std::get<STRIDE>(curr_frame)
...
// 			// If we frames were pushed at the current source, then
// we have completed a path 			if (!frame_pushed) {

// 				// Store it if it the largest path so far.
// 				if (std::get<PATH>(curr_frame).size() >=
// longest_path.size()) { 					longest_path =
...
// 			ordering.erase(std::remove(ordering.begin(),
// ordering.end(), val), ordering.end());
// 		}
...
// std::vector<qop_id>
// Program::get_longest_valid_path(std::vector<std::vector<qop_id>> paths) {
// 	std::vector<qop_id> selected_path;

// 	while (true) {
// 		if (paths.empty()) { return std::vector<qop_id>(); }

// 		auto max_path_it = std::max_element(paths.begin(), paths.end(),
// vec_size_compare); 		selected_path = *max_path_it;
...
// ray_graph Program::generate_ray_graph(const std::vector<qop_id>& level,
// unsigned int qreg_size) { 	ray_graph rg = ray_graph();
...
// 		std::vector<qrane_argumentlist> point_dups =
// std::vector<qrane_argumentlist>();

// 		// Look at all subsequent identifiers
// 		for (auto j = i+1; j != level.end(); ++j) {
// 			qrane_argumentlist j_args = qops[*j]->get_arguments();

// 			if (std::find(point_dups.begin(), point_dups.end(),
// j_args)
// != point_dups.end()) { continue; }

// 			float rise =
// static_cast<float>(j_args.at(1)->get_index()
// - i_args.at(1)->get_index()); 			float run =
// static_cast<float>(j_args.at(0)->get_index() - i_args.at(0)->get_index());

// 			//if (run == 0) { continue; }
// 			if (run == 0) {
// 				key.second =
// std::numeric_limits<float>::infinity(); 			} else {
// key.second = rise / run;
// 			}
...
// 		// For each i, we also add a singleton entry to ensure all
// points can be covered. 		key.second = 0; 		if
// (rg.count(key) == 0) { 			rg[key] = std::deque<qop_id>();
// 		}
// 	}
...
// bool domain_match_by_num(const qrane_domain& dom, const unsigned int&
// search_num) { 	return dom.domain_num == search_num;
// }

// bool comparator_access_graph_domain_size(const std::pair<const
// std::vector<int>, std::pair<lex_type, std::deque<unsigned int>>>& i,
//                                          const std::pair<const
//                                          std::vector<int>,
//                                          std::pair<lex_type,
//                                          std::deque<unsigned int>>>& j,
//                                          domain_map_t& dmp) {
...
// bool comparator_candidate_list_domain_size(const std::pair<lex_type,
// std::vector<unsigned int>>& lhs,
//                                      const std::pair<lex_type,
...
// could be constructed 			std::vector<std::pair<lex_type,
...
// std::end(this->domain_map)); 			if (remaining <
// opt->search_limit) { lim_it = std::next(i, remaining); }
...
// 				long dist_to_rend = std::distance(rit,
// my_combinations.rend()); 				long dist_to_rlimit =
// opt->search_limit; 				if (opt->search_limit == 0) {
// dist_to_rlimit = dist_to_rend; } 				long winner =
// std::min(dist_to_rlimit, dist_to_rend); 				auto
// rlimit = std::next(rit, winner);

// 				for (; rit != rlimit; ++rit) {
// 					if (j->first == rit->second.back()) {
// continue;
// }

// 					// Disqualified if domains have
// differing gate types 					if
// (domains_have_different_gate_type(j->first, rit->second.back())) { continue;
// }

// 					// Disqualified if domains have
// different lex type 					if
// (domains_lex_mismatch(j->first, rit->second.back())) { continue; }

// 					// Disqualified if established lex order
// is
// disobeyed 					if
// (domains_lex_order(rit->second.back(), j->first) != rit->first) { continue; }

// 					// Disqualified if domains are
// inconsistent 					if
// (extended_domain_inconsistent(j->first, rit->second, current_dim)) {
// continue; }
...
// types 				if
// (domains_have_different_gate_type(i->first, j->first)) { continue; }

// 				// Disqualified if domains have different lex
// type 				if (domains_lex_mismatch(i->first,
// j->first)) { continue; }

// 				// Disqualified if domains are inconsistent
// 				if (extended_domain_inconsistent(i->first,
// j->first, current_dim)) { continue; }
...
// 	while (!this->domain_map.empty()) {
// 		std::vector<std::pair<lex_type, std::vector<unsigned int>>>
// all_combinations; 		all_combinations.reserve(2 *
// this->domain_map.size());

// 		for (domain_map_t::iterator i = std::begin(this->domain_map); i
// != std::end(this->domain_map); i++) {

// 			std::vector<std::pair<lex_type, std::vector<unsigned
// int>>> my_combinations;

// 			// First look through all_combinations to see if we can
// extend the domain path
...
// 				// Disqualified if domains have differing gate
// types 				if
// (domains_have_different_gate_type(i->first, pair.second.back())) { continue;
// }

// 				// Disqualified if domains have different lex
// type 				if (i->second.lex !=
// domain_map[pair.second.back()].lex) { continue; }

// 				// Disqualified if established lex order is
// disobeyed
// 				// Think of lhs as "established" and rhs as
// "mergee" 				if
// (domains_lex_order(pair.second.back(), i->first) != pair.first) { continue; }

// 				bool res;
// //#pragma omp critical
// 				res = extended_domain_inconsistent(i->first,
// pair.second, current_dim); 				if (res) { continue; }

// 				pair.second.push_back(i->first);
// 				++limit;
// 			}

// 			// Then we can look through the rest of the domains and
// make
// a
// two-domain path 			domain_map_t::iterator lim_it;
// auto remaining =
// std::distance(i, std::end(this->domain_map)); 			if
// (remaining
// <
// opt->search_limit) { 				lim_it = std::next(i,
// remaining); } else { 				lim_it = std::next(i,
// opt->search_limit);
// 			}
// 			for (auto j = std::next(i, 1); j !=
// std::end(this->domain_map)
// && j != lim_it; ++j) {
...
// 				lex_type lex = domains_lex_order(i->first,
// j->first); 				if (lex == LEX_SUBSET || lex == LEX_NEG)
// { continue; }
...
// bool Program::domains_have_different_gate_type(unsigned int lhs, unsigned int
// rhs) { 	return this->domain_map[lhs].gate_id !=
// this->domain_map[rhs].gate_id;
// };

// bool Program::domains_lex_mismatch(unsigned int lhs, unsigned int rhs) {
...
// lex_type Program::domains_lex_order(unsigned int lhs, unsigned int rhs) {
...
// j.get_time_max()) || 		 (j.get_time_min() > i.get_time_min() &&
// j.get_time_max() < i.get_time_max())) { 		return LEX_SUBSET;
// 	}
...
// 		if (i.lex == LEX_POS) {
// 			if ( (i.get_time_max() < j.get_time_min()) //||
// 		            /* (i.get_time_min() < j.get_time_min() &&
// j.get_time_min() < i.get_time_max() && i.get_time_max() < j.get_time_max())*/
// ) { 				return LEX_POS; 			} else {
// return LEX_SUBSET;
// 			}
...
// i.get_time_min() < j.get_time_max() && j.get_time_max() <
// i.get_time_max())*/) { 				return LEX_NEG;
// } else { 				return LEX_SUBSET;
// 			}
// 		}
// 	}
// 	return LEX_SUBSET;
...
// bool Program::extended_domain_inconsistent(unsigned int new_dom,
// std::vector<unsigned int> path, unsigned int current_dim) {
...
// 	bool result = check_consistency(system, current_dim,
// domain_map[new_dom].args); 	isl_mat_free(system); 	if (result) { return
// false; } // If consistent, return false 	else { return true; } //
// Otherwise, return true
// };

// bool Program::extended_domain_inconsistent(unsigned int lhs, unsigned int
// rhs,
//                                                      unsigned int
//                                                      current_dim) {
...
// 	bool result = check_consistency(my_system, current_dim,
// domain_map[lhs].args); 	isl_mat_free(my_system); 	if (result) {
// return false; }
// // If consistent, return false 	else { return true; } // Otherwise,
// return true
// };

// domain_map_t Program::greedy_domain_selection(candidate_list candidates,
// unsigned int current_dim) { 	domain_map_t new_domain_map;
// 	//new_domain_map.reserve(candidates.size() * 0.5);
// 	bool invalid_matrix;
// 	//bool disrespects_dependences;

// 	while(!candidates.empty()) {
// 		auto max_it = std::max_element(candidates.begin(),
// candidates.end(), std::bind(comparator_candidate_list_domain_size,
...
// 		invalid_matrix = inconsistent_or_rational_matrix(path,
// current_dim); 		if (invalid_matrix) { candidates.erase(max_it);
// continue; }

// 		qrane_domain new_dom = merge_domains(path, max_it->first,
// current_dim); 		new_domain_map[path.front()] = new_dom;

// 		candidates.erase(max_it);
// 		for (auto& pair : candidates) {
// 			for (auto val : pair.second) {
// 				if (std::find(path.begin(), path.end(), val) !=
// path.end()) { 					pair.first = LEX_REMOVE;
// break;
// 				}
// 			}
// 		}
...
// candidates.end(),
// 		[](std::pair<lex_type, std::vector<unsigned int>>& it) { return
// it.first == LEX_REMOVE; }), candidates.end());
// 	}
...
// */
// domain_map_t Program::greedy_domain_selection(access_graph& G, unsigned int
// current_dim) { 	domain_map_t new_domain_map;
// 	//new_domain_map.reserve(G.size() * 0.5);
// 	bool invalid_matrix;

// 	while (!G.empty()) {
// 		auto max_it = std::max_element(G.begin(), G.end(),
// 		              std::bind(comparator_access_graph_domain_size,
// std::placeholders::_1, std::placeholders::_2,
// this->domain_map)); 		std::vector<unsigned int> collection;
// 		std::copy((*max_it).second.second.begin(),
// (*max_it).second.second.end(), std::back_inserter(collection));

// 		//disrespects_dependences =
// deps.path_disrespects_dependences(collection, num_qops);
// 		//if (disrespects_dependences) { G.erase(max_it); continue; }

// 		//cycle_introduced =
// deps.path_introduces_dependence_cycle(collection, num_qops);
// 		//if (cycle_introduced) { G.erase(max_it); continue; }

// 		invalid_matrix = inconsistent_or_rational_matrix(collection,
// current_dim); 		if (invalid_matrix) { G.erase(max_it); continue;
// }

// 		//for (auto val : collection) { std::cout << val << " "; }
// std::cout << std::endl;

// 		qrane_domain new_dom = merge_domains(collection,
// max_it->second.first, current_dim);

// 		//cannot_carry = deps.path_violates_dependences(ctx, collection,
// new_dom, this->domain_map, this->num_domains, false);
// 		//if (cannot_carry) { G.erase(max_it); continue; }

// 		new_domain_map[collection.front()] = new_dom;
// 		//this->deps.update_working_domain(ctx, collection, domain_map,
// new_dom, false);
...
// 				if (std::find(collection.begin(),
// collection.end(), val)
// != collection.end()) {
// to_remove.push_back(it); break;
// 				}
// 			}
// 		}
...
// bool Program::inconsistent_or_rational_matrix(const std::vector<unsigned
// int>& domains, unsigned int current_dim) { 	isl_mat* mat =
// combine_access_matrices(domains, current_dim); 	mat =
// compute_access_relations(mat);

// 	if (!check_consistency(mat, current_dim,
// domain_map[domains.front()].args)) {
// 		//printf("Inconsistent\n");
// 		//printf("----------------\n");
// 		isl_mat_free(mat);
// 		return true;
...
// 	if (!integral_matrix(mat, current_dim,
// domain_map[domains.front()].args)) {
// 		//printf("Rational\n");
// 		//printf("----------------\n");
// 		isl_mat_free(mat);
...
// qrane_domain Program::merge_domains(const std::vector<unsigned int>& domains,
...
// 	new_domain.domain_num = domains.front();	// new_domain takes id
// of the first domain in the list 	new_domain.dimensionality = current_dim;
// // new_domain has dimension equal to current_dim 	new_domain.subdomains =
// domains;            // subdomains is reflexive as well
// 	new_domain.set_local_domain(combine_isl_local_domains(domains,
// current_dim));
...
// __isl_give isl_set* Program::combine_isl_local_domains(const
// std::vector<unsigned int>& domains, unsigned int current_dim) { 	isl_set*
// ret_set = isl_set_empty(isl_space_set_alloc(ctx, 0, current_dim));
//     isl_set* mergee_set;
...
// __isl_give isl_mat* Program::combine_access_matrices(const
// std::vector<unsigned int>& domains, unsigned int current_dim) { 	isl_mat*
// ret_mat = domain_map[domains.front()].get_access_mat_copy();
...
// Program::greedy_nd_domain_selection(std::list<qrane_domain> candidates) {
...
// 		candidates.remove_if(std::bind(is_overlapping,
// std::placeholders::_1, subdomains));
// 	}
// 	return ret;
// };
...
// */
// __isl_give isl_set*
// Program::build_ND_domain_from_sets(std::vector<qrane_domain>& mergees) {
//     isl_set* ret_set = isl_set_empty(isl_space_set_alloc(ctx, 0,
//     mergees.front().dimensionality)); isl_set* mergee_set;
...
//     std::string domain_name = "S" +
//     std::to_string(mergees.front().domain_num); isl_id* id =
//     isl_id_alloc(ctx, domain_name.c_str(), nullptr); ret_set =
//     isl_set_set_tuple_id(ret_set, id); ret_set = isl_set_coalesce(ret_set);
//     return ret_set;
// };
...
// 	mergees[0] 0
// */
// __isl_give isl_mat* Program::compute_access_relations(isl_mat* mat) {
...
// __isl_give isl_mat*
// Program::concat_matrices_dimwise(std::vector<qrane_domain>& mergees) {
// 	isl_mat* merger_mat;
// 	isl_mat* mergee_mat;
...
// 	// Then append an initial merger.consumed column to the mergee
// access_mat 	for (std::size_t i = mergees.size() - 1; i-- > 0;) {
...
// */
// bool Program::integral_matrix(isl_mat* mat, std::size_t dim, std::size_t
// args) { 	isl_mat* solved = isl_mat_copy(mat); 	solved =
// qrane_utils::back_substitute_isl_mat(ctx, solved, args, dim);
...
// 	EXPECTS mat TO BE IN REF
// */
// __isl_keep bool Program::check_consistency(isl_mat* mat, std::size_t dim,
// std::size_t args) { 	isl_size cols = isl_mat_cols(mat); 	isl_val*
// right_val; 	isl_val* left_val;
...
// __isl_give isl_mat* Program::append_column_with_val(__isl_take isl_mat* mat,
// int val) { 	isl_mat* ret = isl_mat_copy(mat); 	isl_size cols =
// isl_mat_cols(ret); 	isl_size rows = isl_mat_rows(ret);
...
//     none
// */
// void Program::build_isl_domain_read_write_schedule() {
...
// deps.generate_write_all_dependences(ctx);
//   	//isl_union_map* computed_deps =
//   isl_union_flow_get_must_dependence(compute_dependences());

//   	//isl_bool subset = isl_union_map_is_subset(scop->dependence,
//   computed_deps);
//   	//isl_bool subset = isl_union_map_is_subset(computed_deps,
//   scop->dependence);
// 	//isl_bool subset = isl_map_is_subset(isl_map_read_from_str(ctx,
// "{[1]}"), isl_map_read_from_str(ctx, "{[1];[2];}"));
//     //std::cout << "SUBSET: " << subset << std::endl;
// };

// isl_union_map* Program::get_dependences() {
...
// */
// __isl_give isl_union_set* Program::initialize_domain() {
// 	isl_union_set* result = isl_union_set_empty_ctx(ctx);
//   	for (auto i = unchanged_domains.begin(); i != unchanged_domains.end();
//   ++i) {
...
// __isl_give isl_union_map* Program::initialize_recovered_schedule() {
// 	isl_union_map* default_schedule = deps.build_implicit_schedule(ctx);
// 	return default_schedule;
// };

// __isl_give isl_union_map* Program::initialize_read_relations() {
// 	isl_union_map* ret = isl_union_map_read_from_str(ctx,
// build_union_read_str().c_str()); 	ret =
// isl_union_map_intersect_domain_union_set(ret,
// isl_union_set_copy(scop->domain)); 	return isl_union_map_coalesce(ret);
// };

// __isl_give isl_union_map* Program::initialize_write_relations() {
// 	isl_union_map* ret = isl_union_map_read_from_str(ctx,
// build_union_write_str().c_str()); 	ret =
// isl_union_map_intersect_domain(ret, isl_union_set_copy(scop->domain));
// return isl_union_map_coalesce(ret);
// };

// __isl_give isl_union_map* Program::initialize_call_relations() {
// 	isl_union_map* ret = isl_union_map_read_from_str(ctx,
// build_union_call_str().c_str()); 	ret =
// isl_union_map_intersect_domain(ret, isl_union_set_copy(scop->domain));
// return isl_union_map_coalesce(ret);
// };

// __isl_give isl_union_map* Program::initialize_arguments_map() {
...
// */
// std::string Program::build_union_read_str() {
...
// 			strm << "[";
// 			for (std::size_t dim = 0; dim < dom.dimensionality;
// ++dim) { 				strm << "i" << dim;
...
// 			strm << "-> q[";
// 			for (std::size_t dim = 0; dim <= dom.dimensionality;
// ++dim) { 				std::string coeff =
// std::to_string(dom.get_coefficient(arg, dim)); if (dim
// != dom.dimensionality) { 					strm << coeff <<
// "*"
// << "i"
// << dim << " + "; 				} else {strm << coeff;}
// 			}
...
// */
// std::string Program::build_union_write_str() {
// 	std::ostringstream strm;
...
// 		for (std::size_t dim = 0; dim <= dom.dimensionality; ++dim) {
// 			std::string coeff =
// std::to_string(dom.get_coefficient(dom.args-1, dim)); if (dim
// !=
// dom.dimensionality) { 				strm << coeff << "*" <<
// "i"
// << dim
...
// std::string Program::build_union_call_str() {
// 	std::ostringstream strm;
...
// void Program::print_isl_domain_read_write_schedule() {
//   std::cout << "Domain := " <<
//   isl_union_set_to_str(isl_union_set_copy(scop->domain)) << std::endl <<
//   std::endl; std::cout << "Read := " <<
//   isl_union_map_to_str(isl_union_map_copy(scop->read)) << std::endl <<
//   std::endl; std::cout << "Write := " <<
//   isl_union_map_to_str(isl_union_map_copy(scop->write)) << std::endl <<
//   std::endl; std::cout << "Call := " <<
//   isl_union_map_to_str(isl_union_map_copy(scop->call)) << std::endl <<
//   std::endl; if (opt->check || opt->schedule_mode != SCHEDULE_IMPLICIT) {
// 	std::cout << "DataDependencies := " <<
// isl_union_map_to_str(isl_union_map_copy(scop->dependence)) << std::endl <<
// std::endl;
//   }
//   std::cout << "RecoveredSchedule := " <<
//   isl_union_map_to_str(isl_union_map_copy(scop->schedule)) << std::endl <<
//   std::endl;
// };

// std::string Program::get_domain_list_str() {
// 	assert(!scop->final_domain_list.empty());
// 	std::ostringstream strm;
//   	for (const auto &dom : scop->final_domain_list) {
...
// std::string Program::generate_codegen_c_str() {
//   	std::string ret;
//   	char* str;
...
//     &qrane_codegen::qrane_stmt_print_user_fprintf_domain, (void*)(this));
...
// &qrane_codegen::qrane_at_each_domain, (void*)(my_scop));

// 	// Build the schedule
// 	isl_ast_node* node = isl_ast_build_node_from_schedule_map(build,
// isl_union_map_copy(my_scop->schedule));
...
// bool Program::check_recovered_respects_dependencies() {
//   bool ret = false;
//   if (opt->check) {

//     // Create a copy of dependencies, default schedule, and recovered
//     schedule isl_union_map* deps = isl_union_map_copy(scop->dependence);
...
//     depInTime = isl_union_map_apply_range(depInTime,
//     isl_union_map_copy(rec));

// 	// deltas(depInTime)
//     //isl_union_map * deltasWithMap = isl_union_map_deltas_map
//     (isl_union_map_copy (depInTime)); isl_set * justDeltas =
//     isl_set_from_union_set (isl_union_map_deltas (depInTime)); isl_set *
//     themin = isl_set_lexmin (justDeltas);
...
// __isl_give isl_union_flow* Program::compute_dependences() {
// 	isl_union_map * all = isl_union_map_union
// (isl_union_map_copy(scop->write), isl_union_map_copy(scop->read));
//     	isl_union_access_info * accinfo =
//     isl_union_access_info_from_sink(isl_union_map_copy(all)); 	accinfo
//     = isl_union_access_info_set_must_source(accinfo, all);
// 	accinfo = isl_union_access_info_set_schedule_map(accinfo,
// isl_union_map_copy(scop->schedule)); 	isl_union_flow * ret =
// isl_union_access_info_compute_flow(accinfo); 	return ret;
// }

// void Program::compute_transformation() {
// 	assert(isl_options_set_schedule_max_constant_term(ctx, 0) ==
// isl_stat_ok); 	assert(isl_options_set_schedule_max_coefficient(ctx, 2)
// == isl_stat_ok); 	switch (opt->schedule_mode)
//     {
// 	  case SCHEDULE_IMPLICIT:
// 	    assert(false);
//       case SCHEDULE_ISL:
//         assert(isl_options_set_schedule_algorithm(ctx,
//         ISL_SCHEDULE_ALGORITHM_ISL) == isl_stat_ok);
// 		if (!opt->quiet) { std::cout << "isl default scheduler
// chosen.\n";
// }
//         break;
...
//         assert (isl_options_set_schedule_algorithm(ctx,
//         ISL_SCHEDULE_ALGORITHM_FEAUTRIER) == isl_stat_ok);
...
//     isl_union_map_copy(scop->dependence));
...
// 	sched_map = isl_union_map_intersect_domain(sched_map,
// this->scop->domain); 	this->scop->schedule = sched_map;
// };

// qrane_output_scop* Program::get_output_scop() {
...
// void Program::modify_substrs(std::vector<Program>& subcircuits) {
// 	// For each substr of the given mainprogram
// 	for (auto& circ : subcircuits) {

// 		substr_new_id_map old_to_new_domains =
// create_old_to_new_domain_map(); 		substr_new_id_map
// old_to_new_qops = create_old_to_new_qop_map(circ.get_qops());

// 		// 4. For each domain in unchanged_domains:
// 		//		- Update the domain nums
// 		//		- Replace the id in the local domain
// 		std::vector<qrane_domain> new_doms =
// this->scop->final_domain_list; 		for (auto& dom : new_doms) {
// unsigned int new_id = old_to_new_domains[dom.domain_num];
// dom.domain_num = new_id; 			std::string new_id_str = "S" +
// std::to_string(new_id);

// 			isl_set* local_domain = dom.get_local_domain_copy();
// 			isl_set_free(dom.local_domain);
// 			local_domain = isl_set_set_tuple_name(local_domain,
// new_id_str.c_str()); dom.set_local_domain(local_domain);
// 		}
...
// 			new_membership[old_to_new_qops[relation.first]] =
// relation.second;
// new_membership[old_to_new_qops[relation.first]].first =
// old_to_new_domains[relation.second.first];
// 		}
...
// substr_new_id_map Program::create_old_to_new_domain_map() {
...
// substr_new_id_map Program::create_old_to_new_qop_map(qrane_statementlist
// stmts) { 	assert(this->statements.size() == stmts.size());
// substr_new_id_map ret; 	ret.reserve(stmts.size());

// 	for (std::size_t i = 0; i < stmts.size(); ++i) {
// 		auto lhs =
// std::dynamic_pointer_cast<qrane_qop>(this->statements.at(i));
...
// std::string Program::get_qasm_string() {
// 	std::ostringstream strm;
// 	strm << "OPENQASM 2.0;\n";
...
// std::string Program::get_qasm_string(std::vector<qop_id> ordering) {
// 	std::ostringstream strm;
// 	strm << "OPENQASM 2.0;\n";
// 	strm << "include \"qelib1.inc\";\n";
// 	strm << "qreg q[" << qreg_size << "];\n";
...
// std::string Program::get_registers() {
//   std::ostringstream strm;
...
// std::string Program::get_domain_profile_str() {
// 	assert(!scop->final_domain_list.empty());
// 	std::ostringstream strm;
...
// 		dims_info[scop->final_domain_list[i].dimensionality].second +=
// dom_card;
// 	}
...
// std::string Program::print_reconstruction_histogram(std::vector<qrane_domain>
// dom_list) {
...
// std::string Program::print_domain_size_histogram(std::vector<qrane_domain>
// dom_list) { 	std::map<unsigned int, unsigned int> hist; 	for (auto dom :
// dom_list) { 		hist[dom.stmt_count] += 1;
// 	}
