...
typedef std::map<qop_id, std::pair<unsigned int, std::vector<unsigned int>>>
    membership_map_t;
typedef std::map<std::string, qop_id> membership_map_reverse;

typedef std::unordered_map<qop_id, std::vector<std::pair<qop_id, int>>>
    stride_graph;

typedef std::unordered_map<std::pair<qop_id, float>, std::deque<qop_id>,
                           PairHasher>
    ray_graph;
