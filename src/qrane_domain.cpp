...
/*
	This is the main one
*/
void qrane_domain::build_affine_list_from_qop_ids(qasm_ops_list& qops_list, std::vector<unsigned int>& qop_ids) {
	if (stmt_count == 1) {
		std::vector<int> op_0_args = std::get<ARGS>(qops_list[qop_ids[0]]);
		for (int i = 0; i < args; ++i) { affine_list.push_back(std::make_pair(0, op_0_args[i])); }
	} else {
		std::vector<int> op_0_args = std::get<ARGS>(qops_list[qop_ids[0]]);
...
void qrane_domain::add_component_qops(std::vector<unsigned int> component_qops) {
