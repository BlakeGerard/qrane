...
isl_stat qrane_codegen::qrane_build_domain_key(__isl_take isl_ast_expr *ae,
                                               void *usr) {
...
__isl_give isl_printer *qrane_codegen::qrane_stmt_print_user_fprintf_domain(
    __isl_take isl_printer *prt, __isl_take isl_ast_print_options *options,
    __isl_keep isl_ast_node *node, void *usr) {
  isl_id *id = isl_ast_node_get_annotation(node);
  isl_ast_expr_list *gate_call_expr_list =
      (isl_ast_expr_list *)(isl_id_get_user(id));
  ...
  isl_stat res = isl_ast_expr_list_foreach(
      gate_call_expr_list, &qrane_codegen::qrane_build_domain_key,
      (void *)(key));
...
*/
isl_stat qrane_codegen::qrane_build_gate_operation_fprintf_qasm(
    __isl_take isl_ast_expr *ae, void *usr) {
...
__isl_give isl_printer *qrane_codegen::qrane_stmt_print_user_fprintf_qasm(
    __isl_take isl_printer *prt, __isl_take isl_ast_print_options *options,
    __isl_keep isl_ast_node *node, void *usr) {
  isl_id *id = isl_ast_node_get_annotation(node);
  isl_ast_expr_list *gate_call_expr_list =
  ...
  cnt = 0;
  isl_stat res;
  res = isl_ast_expr_list_foreach(
      gate_call_expr_list,
      &qrane_codegen::qrane_build_gate_operation_fprintf_qasm, (void *)(prt));
...
*/
isl_stat
qrane_codegen::qrane_build_gate_operation_default(__isl_take isl_ast_expr *ae,
                                                  void *usr) {
  isl_printer *prt = (isl_printer *)(usr);
...
__isl_give isl_printer *qrane_codegen::qrane_stmt_print_user_default(
    __isl_take isl_printer *prt, __isl_take isl_ast_print_options *options,
    __isl_keep isl_ast_node *node, void *usr) {
  isl_id *id = isl_ast_node_get_annotation(node);
  isl_ast_expr_list *gate_call_expr_list =
  ...
  isl_stat res;
  res = isl_ast_expr_list_foreach(
      gate_call_expr_list, &qrane_codegen::qrane_build_gate_operation_default,
      (void *)(prt));
  assert(res == isl_stat_ok);
  prt = isl_printer_end_line(prt);
...
*/
isl_stat
qrane_codegen::qrane_build_single_access_expression(__isl_take isl_map *acc,
                                                    void *usr) {
  ...
  // isl_id* q =
  // isl_id_alloc(isl_ast_build_get_ctx(isl_ast_build_copy(info->build)), "q",
  // nullptr);
  ...
  for (int i = 0; i < nd; ++i) {
    ...
    isl_ast_expr *aexp;
    aexp = isl_ast_build_access_from_multi_pw_aff(
        info->build, isl_multi_pw_aff_from_pw_aff(pwa));
...
/*
        ...
        parameters.
*/
__isl_give isl_ast_expr_list *
qrane_codegen::qrane_build_access_at_domain(__isl_keep isl_ast_build *build,
  ...
  res = isl_union_map_foreach_map(
      sched_to_call, &qrane_codegen::qrane_build_single_access_expression,
      (void *)(&info));
  assert(res == isl_stat_ok);
...
qrane_codegen::qrane_at_each_domain(__isl_keep isl_ast_node *node,
  ...
  std::string gate_id =
      qrane_codegen::find_domain_gate_id(dom_name, scop->final_domain_list);

  isl_ast_expr_list *acc_expr_list;
  acc_expr_list =
      qrane_codegen::qrane_build_access_at_domain(build, sched, scop->call);
