...
namespace qrane {
    ...
    template <class T>
    ParameterValue<T>::ParameterValue(T value) {
        value_ = value;
    }

    template <class T>
    std::string ParameterValue<T>::to_string() const {
    ...
    void BinaryExpr::set_type(binary_expr_e type) {
        type = type;
    }

    void BinaryExpr::set_lhs(std::shared_ptr<Parameter> lhs) {
        lhs = lhs;
    }

    void BinaryExpr::set_rhs(std::shared_ptr<Parameter> rhs) {
        rhs = rhs;
    }
    ...
    UnaryExpr::UnaryExpr(unary_expr_e type, std::string op, std::shared_ptr<Parameter> arg) {
        type_ = type;
        op_ = op;
        arg_ = arg;
    }

    void UnaryExpr::set_type(unary_expr_e type) {
        type_ = type;
    }

    void UnaryExpr::set_arg(std::shared_ptr<Parameter> arg) {
        arg_ = arg;
    }
