...
qrane_host::circuit_decomposition(aquma_graph *ag, aquma_circuit *circ) {
  std::set<int> pending;
  ...
  while (!ag->m_qsubstr.empty()) {
    ...
    // If the chosen substr is below the bound, all other substrs will be that
    // small. The pool of remaining gates is chunked like normal
    if (selected.first.size() <= DECOMP_SUBSTR_SIZE_BOUND) {
      std::vector<qrane_mainprogram> mps =
          generate_qrane_mainprogram_list_from_chunked_statements(
              qrane_stmts, subcircuit_count);
    ...
    if (selected.first.size() > max_substr_size) {
      max_substr_size = selected.first.size();
    }

    // Now remove the substrings from the m_qsubstr and qrane_stmts lists
    ag->splice_substr(selected.second, selected.first.size());
    qrane_utils::splice_stmtlist(qrane_stmts, selected.second,
                                 selected.first.size());
    subcircuit_count += 1;
  }
  ...
  while (depth <= RECURSION_DEPTH_BOUND) {
    ...
    if (curr_locs.size() < RECURSION_SUBSTR_OCCURRENCE_BOUND) {
      if (!prev.empty()) {
        ...
        return qrane_substr_info(curr, curr_locs);
      }
    }
...
qrane_mainprogram qrane_host::generate_qrane_mainprogram_from_aquma_substr(
  ...
  std::vector<qrane_statement *>::const_iterator first =
      stmts_2Q.begin() + info.second[0];
  std::vector<qrane_statement *>::const_iterator last =
      stmts_2Q.begin() + info.second[0] + info.first.size();
...
void qrane_host::store_substrs_in_qrane_mainprogram(
  ...
  substrs.reserve(info.second.size() - 1);

  for (std::size_t i = 1; i < info.second.size(); ++i) {
    std::vector<qrane_statement *>::const_iterator first =
        stmts_2Q.begin() + info.second[i];
    std::vector<qrane_statement *>::const_iterator last =
        stmts_2Q.begin() + info.second[i] + info.first.size();
    std::vector<qrane_statement *> stmts(first, last);
...
std::vector<qrane_mainprogram>
qrane_host::generate_qrane_mainprogram_substrs(qrane_mainprogram &mp) {
  std::vector<qrane_mainprogram> new_mps;
  new_mps.reserve(mp.substrs.size());
  ...
  for (const auto &stmts : mp.substrs) {
    ...
    qrane_mainprogram substr_mp =
        create_fresh_qrane_mainprogram(stmts->get_2Q_gates(), 9999);
    ...
    for (const auto &relation : substr_mp.get_deps().membership) {
      ...
      new_membership[qops[relation.first]].first =
          domains[relation.second.first];
    }
...
qrane_host::get_new_domain_ids_list(unsigned int cnt) {
  num_domains += 1;
...
substr_new_id_map
qrane_host::create_old_to_new_domain_map(qrane_mainprogram &mp) {
  ...
  ret.reserve(new_domain_ids.size());
...
substr_new_id_map qrane_host::create_old_to_new_qop_map(qrane_mainprogram &mp,
                                                        qrane_stmtlist *stmts) {
  std::vector<qrane_statement *> old_qop_ids =
      mp.get_stmtlist()->get_2Q_gates();
  std::vector<qrane_statement *> new_qop_ids = stmts->get_2Q_gates();
  assert(old_qop_ids.size() == new_qop_ids.size());

  substr_new_id_map ret;
  ret.reserve(old_qop_ids.size());
  for (std::size_t i = 0; i < old_qop_ids.size(); ++i) {
    ret[((qrane_qop *)old_qop_ids[i])->get_dim1_qop_num()] =
        ((qrane_qop *)new_qop_ids[i])->get_dim1_qop_num();
  }
  return ret;
}
...
int qrane_host::process_circuit() {
  if (opt->chunk > 1) {
    ...
    auto mps =
        generate_qrane_mainprogram_list_from_chunked_statements(stmts, 0);
...
void codegen_for_all_subcircuits(std::vector<qrane_mainprogram> mps) {
  ...
  for (auto &mp : mps) {
    ...
    std::string codegen_file =
        std::string("sabretest/gen_" + std::to_string(ctr) + ".qasm");
    ...
    int res = qrane_utils::generate_c_test_file(
        c_name, qasm_name, codegen_c_str, mp.get_registers());
    if (res) {
...
void qrane_host::merge_qubit_access_profiles(qubit_profile_map &a,
                                             qubit_profile_map &b) {
...
int qrane_host::run_checks() {
  ...
  if (!opt->quiet) {
    std::cout << "Checking that control-read dependences are subset of "
                 "control-write dependence ... "
              << std::flush;
  }
  check_res = check_qubit_access_profile_equivalence();
  if (check_res == false) {
...
bool qrane_host::check_qubit_access_profile_equivalence() {
  ...
  qrane_utils::generate_c_test_file(c_name, qasm_name, codegen_c_str,
                                    main_processor->get_registers());
  ...
  // We need to do this so that checkpg->get_qubit_profile_map() has something
  // to create
...
void qrane_host::print_stats() {
  ...
  if (opt->chunk > 1 || opt->substr) {
    std::cout << "Qops: " << main_processor->get_num_qops() + 1 << std::endl;
  } else {
...
int qrane_host::output_to_files() {
  ...
  if (!opt->codegen_file.empty()) {
    ...
    std::string codegen_c_str = main_processor->generate_codegen_c_str();
    fs::path codegen_file = fs::absolute(opt->codegen_file);
