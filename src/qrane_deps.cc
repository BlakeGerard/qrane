...
qrane_deps::qrane_deps(std::vector<qrane_statement *> stmts) {
  ...
  for (auto val : stmts) {
    ...
    membership[qop->get_dim1_qop_num()] =
        std::make_pair(qop->get_dim1_qop_num(), std::vector<unsigned int>());
  }
};
...
membership_map qrane_deps::get_membership() const { return this->membership; }
...
*/
void qrane_deps::add_vertex(unsigned int line_num, int index_accessed,
                            char type, std::string gate_id) {
    ...
    if (latest_access[index_accessed].size() > 0) {
      ...
      if (current_access.second == 'r') {
        ...
        if (latest_access[index_accessed].front().second == 'w') {
          ...
          if (queue_member.first == current_access.first) {
            return;
          }
...
void qrane_deps::add_write_all_vertex(unsigned int line_num, int index_accessed,
                                      char type, std::string gate_id) {
  auto current_access = std::pair<unsigned int, char>(line_num, type);
  std::pair<unsigned int, char> queue_member;
    ...
    if (latest_access_write_all[index_accessed].size() > 0) {
      ...
      if (current_access.second == 'r') {
        ...
        if (latest_access_write_all[index_accessed].front().second == 'w') {
                ...
                // Generate a dependence only. The access gets pushed onto the
        queue queue_member = latest_access_write_all[index_accessed].front(); if
        (queue_member.first == current_access.first) { return; }
                ddg[queue_member.first].push_back(current_access.first);
...
/*
        TODO: Document
*/
void qrane_deps::set_membership(unsigned int line_num, unsigned int index,
                                unsigned int domain_num) {
  auto rhs = std::pair<unsigned int, std::vector<unsigned int>>(
      domain_num, std::vector<unsigned int>{index});
...
*/
void qrane_deps::update_membership_graph_1D_phase(
    const std::vector<unsigned int> &path, unsigned int domain_num) {
...
*/
void qrane_deps::update_membership_graph_ND_phase(
    const std::vector<unsigned int> &domains, domain_map_t &domain_map) {
  unsigned int parent_num = domains.front();
  std::vector<unsigned int> components = domain_map[parent_num].component_qops;

  // Update the points of the parent domain with a zero value for outermost loop
  // variable
  for (auto val : components) {
    membership[val].second.push_back(0);
  }

  // Do the same as above for each subsequent domain, but use mergee_counter
  // instead of zero
...
isl_union_map *
qrane_deps::build_implicit_schedule(isl_ctx *ctx, unsigned int subcircuit_num) {
...
__isl_keep isl_union_map *qrane_deps::generate_validity_map(isl_ctx *ctx) {
  ...
  for (const auto &vertex : ddg) {
    ...
    for (const auto neighbor : vertex.second) {
      ...
      for (std::vector<unsigned int>::reverse_iterator dim =
               left_info.second.rbegin();
           dim != (left_info.second.rend() - 1); ++dim) {
        strm << std::to_string(*dim) << ",";
      }
      ...
      for (std::vector<unsigned int>::reverse_iterator dim =
               right_info.second.rbegin();
           dim != (right_info.second.rend() - 1); ++dim) {
        strm << std::to_string(*dim) << ",";
      }
      ...
      ret_map =
          isl_union_map_coalesce(isl_union_map_add_map(ret_map, single_map));
      strm.str(std::string());
    }
  }
...
qrane_deps::generate_write_all_dependences(isl_ctx *ctx) {
  std::ostringstream strm;
  isl_union_map *ret_map = isl_union_map_empty_ctx(ctx);
  isl_map *single_map;
...
/*
        TODO: Document
*/
void qrane_deps::decrement_in_degrees_of_path_neighbors(
...
std::array<unsigned int, 2> qrane_deps::dependence_graph_lookahead_depth(
  ...
  for (auto i = frontier.begin(); i != frontier.end(); ++i) {
    ...
    count[args - 1] += 1;
    dependence_graph_lookahead_depth_main(*i, num_qops, depth, count,
                                          qops_list);
  }
  return count;
};
...
void qrane_deps::dependence_graph_lookahead_depth_main(
    unsigned int src, unsigned int num_qops, unsigned int depth,
    std::array<unsigned int, 2> &count, qasm_ops_list &qops_list) {
...
std::string qrane_deps::get_gates_networkx_edge_list_str(
...
void qrane_deps::print_membership() {
  ...
  for (const auto &entry : membership) {
    ...
    for (auto r = entry.second.second.rbegin(); r != entry.second.second.rend();
         ++r) {
      stream << *r << " ";
    }
    stream << "])" << std::endl;
