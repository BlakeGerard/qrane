...
namespace qrane {
	...
	DependenceProfile::DependenceProfile(const std::map<qop_id, std::shared_ptr<Qop>>& qops,
		const qrane_options* opt) {
		auto V = utils::get_map_keys_as_initializer_list<qop_id, std::shared_ptr<Qop>>(qops);
		G_static_ = Graph<qop_id>(V, true);
		G_dynamic_ = Graph<qop_id>(V, true);
		latest_access_ = std::map<qop_id, std::queue<std::pair<qop_id, access_type_e>>>();
	...
	*/
	void DependenceProfile::construct_dependence_graph(const std::map<qop_id, std::shared_ptr<Qop>>& qops,
													   const qrane_options* opt) {
		...
		for (const auto& element : qops) {
			...
			for (unsigned int i = 0; i < num_args; ++i) {
				...
				if (opt->write_all) type = WRITE;

				// Otherwise, only the last argument is a write
				else {
					if (i < num_args - 1) type = READ; 
					else type = WRITE;
				}
				add_dependence(element.second->id(), element.second->arg(i)->index(), type);
			}
		}
	}

	void DependenceProfile::add_dependence(const qop_id accessor, const qubit_id accessed, 
			...
			if (latest_access_[accessed].size() > 0) {
				...
				if (type == READ) {
					...
					if (latest.second == WRITE) {
						...
						if (latest.first == accessor) { return; }
				...
				// If the current access is a write (RAW or WAW)
					...
					while (!latest_access_[accessed].empty()) {
						...
						if (latest.first == accessor) { continue; }
						G_static_.add_edge(latest.first, accessor);
						G_dynamic_.add_edge(latest.first, accessor);
					}
				}
	...
	void DependenceProfile::remove_frontier_subset(const adjacency_t<qop_id>& vertices) {
		G_dynamic_.remove_vertices(vertices);
	}

	std::array<unsigned int, 2> DependenceProfile::lookahead_breadth(unsigned int breadth, 
		std::vector<qop_id>& frontier, std::map<qop_id, std::shared_ptr<Qop>>& qops) {
	...
	std::array<unsigned int, 2> DependenceProfile::lookahead_depth(unsigned int depth, 
		std::vector<qop_id>& frontier, std::map<qop_id, std::shared_ptr<Qop>>& qops) {
	...
	void DependenceProfile::lookahead_depth_main(unsigned int depth, qop_id src,  
	...
	/*
			This function builds a string representation of the point-to-point
		dependencies in ISL format. Each qop_id in G_static_ is converted
		to its associated point of an iteration domain, e.g.:
			qop_id 4 -> S2[0,1]

	*/
	isl_union_map* DependenceProfile::build_implicit_schedule(isl_ctx* ctx, const membership_map_t& membership) {
