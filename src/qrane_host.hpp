...
private:
  ...
  qrane_program create_fresh_qrane_program(qrane_statementlist stmts);
  ...
  void
  modify_subcircuits(std::vector<std::vector<qrane_program>::iterator> its);
  std::vector<qrane_program>
  qrane_programs_from_substr_result(qrane_substr_result &result,
                                    qrane_statementlist &statements_2Q);
  qrane_statementlist
  collect_unpartitioned_statements(std::vector<qrane_program> &subcircuits);
