...
Host::Host(qrane_options *opt, qrane::global_timer_s *timer) {
  this->opt = opt;
  this->timer = timer;
  this->main_processor = std::make_shared<Program>(opt);
  this->circuit_count = 0;
};

int Host::parse_circuit() {
...
int Host::process_circuit_via_substring_decomposition() {
  ...
  // Qrane decomposition
  auto all = std::vector<Program>();
  auto subcircuits = circuit_decomposition(ag, circ);
  auto remaining = generate_Program_list_from_chunked_statements(
  ...
  if (opt->schedule_mode != SCHEDULE_IMPLICIT) {
    main_processor->compute_transformation();
  }
...
qrane_statementlist
Host::collect_unpartitioned_statements(std::vector<Program> &subcircuits) {
  ...
  for (const auto &statement : all_statements) {
    if (statement->is_qop()) {
      ...
      for (auto &circ : subcircuits) {
        ...
        bool c2 = qop->get_id() > circ.get_time_min() &&
                  qop->get_id() < circ.get_time_max();
        if (c0 || c1 || c2) {
          continue_flag = true;
          break;
        }
...
std::vector<Program> Host::circuit_decomposition(aquma_graph *ag,
                                                 aquma_circuit *circ) {
  ...
  while (!ag->m_qsubstr.empty()) {
    ...
    std::vector<Program> subcircuits =
        Programs_from_substr_result(result, statements_2Q);
    all.insert(all.end(), subcircuits.begin(), subcircuits.end());

    // Now remove the substrings from the m_qsubstr and qrane_stmts lists
    ag->splice_substr(result.second, result.first.size());
    statements_2Q = qrane_utils::splice_stmtlist(statements_2Q, result.second,
                                                 result.first.size());
  }
  return all;
}

qrane_substr_result Host::substr_recursion(aquma_graph *ag) {
  std::vector<int> prev;
  ...
  while (depth <= RECURSION_DEPTH_BOUND) {
    ...
    if (curr.empty()) {
      return qrane_substr_result(ag_prime.m_qsubstr,
...
std::vector<Program>
Host::programs_from_substr_result(qrane_substr_result &result,
  ...
  for (std::size_t i = 0; i < result.second.size(); ++i) {
    ...
    Program mp = create_fresh_Program(in_range);
    if (i == 0) {
      leader = mp.get_id();
      this->subcircuit_map.insert(
          std::make_pair(leader, std::vector<circuit_id>()));
    } else {
...
std::vector<Program> Host::generate_program_list_from_chunked_statements(
  ...
  std::vector<Program> mps;
  mps.reserve(chunked.size());
  for (auto &chunk : chunked) {
    mps.push_back(create_fresh_Program(chunk));
  }
  return mps;
}
...
*/
Program Host::create_fresh_Program(qrane_statementlist statements) {
  Program new_mp = Program(opt, this->circuit_count);
  new_mp.initialize(statements, main_processor->qreg_size);
  this->circuit_count += 1;
  return new_mp;
}

aquma_circuit *Host::read_aquma_circuit_from_qasm() {
  int retval;
  char cmdstr[1000];
...
int Host::set_fixed_aquma_options(aquma_options *aopt) {
  if (this->opt->calibration_file == NULL) {
    std::cout
        << "You must provide a device_file to utilize Aquma facilities.\n";
    return 1;
  }
...
int Host::process_circuit() {
  if (opt->chunk > 1) {
    auto qops = main_processor->get_statementlist().get_qops();
    auto mps = generate_Program_list_from_chunked_statements(qops);
...
void Host::sequential_process() {
  main_processor->parse_domains();
  if (!opt->quiet) {
    std::cout << "Merging affine abstractions ... " << std::flush;
  }
  main_processor->build_isl_domain_read_write_schedule();
};

bool comparator_mainprogram(Program &a, Program &b) {
  return a.get_time_max() < b.get_time_min();
};

void Host::sort_mainprograms(std::vector<Program> &mps) {
  std::sort(mps.begin(), mps.end(), comparator_mainprogram);
};

void Host::testing(std::vector<Program> mps) {
  // Sort mainprograms so they are in correct order
  sort_mainprograms(mps);

  // Initialize qrane_routing
  qrane_routing router =
      qrane_routing(mps[0].get_dependence_graph(), opt->coupling_file);
  ...
  for (const auto &mp : mps) {
    ...
    results.push_back(
        router.export_to_tket(std::make_shared<Program>(mp), opt));
  ...
  if (results.size() == 1) {
    ...
    for (std::size_t i = 0; i < results.size() - 1; ++i) {
      ...
      std::cout << "Reconciling " << i << " and " << i + 1 << " ... \n"
                << std::flush;
      auto swaps = router.reconcile_mappings(results[i].second.second,
                                             results[i + 1].second.first);
      std::cout << "done\n";
  ...
  Program full = Program(opt);
  full.initialize(all, mps[0].qreg_size);

  std::cout << std::boolalpha
            << router.simulate_circuit_execution(all, results[0].second.first)
            << std::endl;
};

void Host::modify_subcircuits(std::vector<std::vector<Program>::iterator> its) {
  ...
  for (auto i = its.begin() + 1; i < its.end(); ++i) {
    ...
    substr_new_id_map old_to_new_domains =
        (*leader)->create_old_to_new_domain_map();
    substr_new_id_map old_to_new_qops =
        (*leader)->create_old_to_new_qop_map((*i)->get_qops());
    ...
    std::vector<qrane_domain> new_doms =
        (*leader)->get_scop()->final_domain_list;
...
void Host::parallel_process(std::vector<Program> &mps) {
  unsigned int qreg_size = main_processor->qreg_size;
  std::vector<t_qrane_scop *> all_scops =
      std::vector<t_qrane_scop *>(mps.size());
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (const auto &entry : this->subcircuit_map) {
    std::vector<std::vector<Program>::iterator> temp;
    ...
    for (auto mp = mps.begin(); mp != mps.end(); ++mp) {
      if (std::find(entry.second.begin(), entry.second.end(), mp->get_id()) !=
          entry.second.end()) {
        temp.push_back(mp);
      }
    }
  ...
  for (std::size_t i = 0; i < all_scops.size(); ++i) {
    full_scop->domain =
        isl_union_set_union(full_scop->domain, all_scops[i]->domain);
    full_scop->read = isl_union_map_union(full_scop->read, all_scops[i]->read);
    full_scop->write =
        isl_union_map_union(full_scop->write, all_scops[i]->write);
    full_scop->call = isl_union_map_union(full_scop->call, all_scops[i]->call);
    full_scop->schedule =
        isl_union_map_union(full_scop->schedule, all_scops[i]->schedule);
    full_scop->dependence =
        isl_union_map_union(full_scop->dependence, all_scops[i]->dependence);
    full_scop->arguments =
        isl_union_map_union(full_scop->arguments, all_scops[i]->arguments);
    full_scop->membership.insert(all_scops[i]->membership.begin(),
                                 all_scops[i]->membership.end());
    merge_qubit_access_profiles(full_scop->qubit_access_profile,
                                all_scops[i]->qubit_access_profile);
    std::move(all_scops[i]->final_domain_list.begin(),
              all_scops[i]->final_domain_list.end(),
...
void Host::merge_qubit_access_profiles(qubit_profile_map &a,
                                       qubit_profile_map &b) {
...
int Host::run_checks() {
  ...
  if (!opt->quiet) {
    std::cout << "Checking that all qops have been accounted for ... "
              << std::flush;
  }
  ...
  if (!opt->quiet) {
    std::cout << "Checking that recovered schedule respects dependences ... "
              << std::flush;
  }
  ...
  if (!opt->quiet) {
    std::cout
        << "Checking that all qubits have the same number of accesses ... "
        << std::flush;
  }
  ...
  if (!opt->quiet) {
    std::cout << "Checking for isomorphism between original and recovered "
                 "dependence graphs ... "
              << std::flush;
  }
...
bool Host::check_qubit_access_profile_equivalence() {

  // Write the mainpg codegen to a C file, compile, and generate check qasm
  // file.
  ...
  qrane_utils::generate_c_test_file(
      c_name, qasm_name, codegen_c_str, main_processor->get_registers(),
      main_processor->get_membership_map_reverse_str());
  ...
  this->check_processor = std::make_shared<Program>(opt);
...
int Host::check_input_output_isomorphism() {
  this->main_processor = std::make_shared<Program>(opt);
  this->check_processor = std::make_shared<Program>(opt);
...
bool Host::check_isomorphism() {
  ...
  std::string clean =
      "rm -f " + main_graph_path + " " + check_graph_path + " " + result_path;
  ...
  if (graph_out.is_open()) {
    ...
    std::cout << "Failed to open main circuit NetworkX input file."
              << std::endl;
    return false;
  }
  ...
  if (graph_out.is_open()) {
    ...
    std::cout << "Failed to open check circuit NetworkX input file."
              << std::endl;
    return false;
  }
  ...
  command << "from networkx import DiGraph, read_edgelist, is_isomorphic"
          << std::endl;
  command << "def node_match(n1, n2):" << std::endl;
  command << "\t return n1 == n2" << std::endl;
  command << "G1 = read_edgelist(\'" << main_graph_path
          << "\', create_using=DiGraph())" << std::endl;
  command << "G2 = read_edgelist(\'" << check_graph_path
          << "\', create_using=DiGraph())" << std::endl;
...
void Host::print_scop() {
  if (!opt->quiet) {
    main_processor->print_isl_domain_read_write_schedule();
  }
  if (!opt->quiet && opt->membership) {
    main_processor->print_membership_graph();
  }
};

void Host::print_stats() {
  ...
  std::cout << "Total: " << main_processor->get_num_domains() << " domains, "
  ...
  std::cout << "Reconstruction: "
            << std::chrono::duration_cast<std::chrono::milliseconds>(
                   timer->reconstruction_complete - timer->global_start)
                   .count()
            << "ms" << std::endl;
  std::cout << "Scheduling: "
            << std::chrono::duration_cast<std::chrono::milliseconds>(
                   timer->scheduling_complete - timer->reconstruction_complete)
                   .count()
            << "ms" << std::endl;
  if (opt->check) {
    std::cout << "Verification: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(
                     timer->checking_end - timer->checking_start)
                     .count()
              << "ms" << std::endl;
  }
  std::cout << "Total execution time: "
            << std::chrono::duration_cast<std::chrono::milliseconds>(
                   timer->global_end - timer->global_start)
                   .count()
            << "ms" << std::endl;
  std::cout << "-------------" << std::endl;
}

qrane_output_scop *Host::get_scop() {
  return main_processor->get_output_scop();
};

int Host::output_to_files() {
  if (opt->aquma_file) {
  ...
  if (opt->codegen_file) {
    ...
    int res = qrane_utils::generate_c_test_file(
        c_name, qops_name, codegen_c_str, main_processor->get_registers(),
        main_processor->get_membership_map_reverse_str());
    if (res) {
...
int Host::parse_options(int argc, char *argv[]) {
  int gopt;
  ...
  while (1) {
    ...
    case 19:
      if (optarg) {
        ...
        std::cout << "Must provide an edge-list formatted coupling graph .txt "
                     "file.\n";
        return 1;
      }
      break;
