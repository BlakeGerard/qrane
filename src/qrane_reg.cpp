...
Reg::Reg(element_variant_e element_variant, reg_variant_e reg_variant,
         std::string device, std::string name,
         std::initializer_list<qubit_id> reg)
    : Element(element_variant) {
  variant_ = reg_variant;
  device_ = device;
  name_ = name;
...
*/
Reg::Reg(element_variant_e element_variant, reg_variant_e reg_variant,
         std::string device, std::string name, qop_id start, qop_id stop)
    : Element(element_variant) {
  variant_ = reg_variant;
  device_ = device;
  name_ = name;
...
qubit_id Reg::operator[](unsigned int index) const { return reg_.at(index); };
...
qubit_id Reg::at(qubit_id id) const { return reg_.at(id); }

std::string Reg::name() const { return name_; }

unsigned int Reg::size() const { return reg_.size(); }

bool Reg::count(qubit_id id) const { return reg_.count(id); };

bool Reg::is_qreg() const { return variant_ == reg_variant_e::QREG; };

bool Reg::is_creg() const { return variant_ == reg_variant_e::CREG; };
...
void Reg::insert(qubit_id id) { reg_.insert(std::make_pair(id, id)); }
...
reg_variant_t::iterator Reg::begin() { return reg_.begin(); };

reg_variant_t::iterator Reg::end() { return reg_.end(); };

reg_variant_t::const_iterator Reg::begin() const { return reg_.cbegin(); };

reg_variant_t::const_iterator Reg::end() const { return reg_.cend(); };
