...
unsigned int Qop::num_params() const {
	return params_.size();
}

unsigned int Qop::num_args() const {
	return args_.size();
}
...
std::vector<std::shared_ptr<Parameter>> Qop::parameters() {
	return params_;
}

std::vector<std::shared_ptr<Argument>> Qop::arguments() {
	return args_;
}
...
std::shared_ptr<Qop> Qop::make_unassigned_2Q_gate(std::string name, 
	...
	auto ret = Qop(element_variant_e::QOP, qop_variant_e::GATE, -1, name, {q_arg0, q_arg1});
	return std::make_shared<Qop>(ret);
}

bool Qop::is_1Q_gate() const {
	return args_.size() == 1;
}

bool Qop::is_2Q_gate() const {
	return args_.size() == 2;
}
