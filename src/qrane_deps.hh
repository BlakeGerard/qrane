...
typedef std::unordered_map<int, std::queue<std::pair<unsigned int, char>>>
    access_map;
...
public:
  ...
  void add_vertex(unsigned int line_num, int index_accessed, char type,
                  std::string gate_id);
  void add_write_all_vertex(unsigned int line_num, int index_accessed,
                            char type, std::string gate_id);
  void set_membership(unsigned int line_num, unsigned int index,
                      unsigned int domain_num);
  void update_membership_graph_1D_phase(const std::vector<unsigned int> &path,
                                        unsigned int domain_num);
  void
  update_membership_graph_ND_phase(const std::vector<unsigned int> &domains,
                                   domain_map_t &domain_map);

  std::array<unsigned int, 2>
  dependence_graph_lookahead_breadth(std::vector<unsigned int> &frontier,
                                     unsigned int num_qops, unsigned int depth,
                                     qasm_ops_list &qops_list);
  std::array<unsigned int, 2>
  dependence_graph_lookahead_depth(std::vector<unsigned int> &frontier,
                                   unsigned int num_qops, unsigned int depth,
                                   qasm_ops_list &qops_list);
  void dependence_graph_lookahead_depth_main(unsigned int src,
                                             unsigned int num_qops,
                                             unsigned int depth,
  ...
  std::string
  get_gates_networkx_edge_list_str(std::vector<qrane_statement *> qops_list);
