...
Statement::Statement(isl_ctx *ctx, unsigned int id_,
  ...
  if (component_qops_.size() == 1) {
    lex_ = LEX_SINGLE;
  } else if (component_qops_[1] - component_qops_[0] > 0) {
    lex_ = LEX_POS;
  } else if (component_qops_[1] - component_qops_[0] < 0) {
    lex_ = LEX_NEG;
  } else {
...
/*
        This is the main one
*/
void Statement::build_affine_list_from_qop_ids(
    const std::map<qop_id, std::shared_ptr<Qop>> &qops,
    std::vector<qop_id> &qop_ids) {
  if (card_ == 1) {
...
void Statement::raise_dimensionality() { dim_ += 1; }

void Statement::set_dimensionality(unsigned int dim) { dim_ = dim; }

void Statement::set_domain(isl_set *new_domain) { domain_ = new_domain; }

void Statement::set_access_mat(isl_mat *mat) { access_mat_ = mat; }
...
__isl_keep void Statement::build_local_domain(isl_ctx *ctx) {
  ...
  int rows = card_;
  int cols = dim_ + 2 * (num_args_); // 1 column for i and 2 columns per arg(1
                                     // offset and 1 value)
  ...
  for (int row = rows; row-- > 0;) {
    ...
    for (int ctr = num_args_; ctr-- > 0;) {
      ...
      access_mat_ = isl_mat_set_element_si(access_mat_, row, index,
                                           affine_list_[fnct].first * val +
                                               affine_list_[fnct].second);
    }
  }
}
...
unsigned int Statement::card() const { return component_qops_.size(); }

unsigned int Statement::dim() const { return dim_; }
...
void Statement::back_substitution(isl_ctx *ctx) {
  ...
  access_mat_ =
      utils::back_substitute_isl_mat(ctx, access_mat_, num_args_, dim_);
}
