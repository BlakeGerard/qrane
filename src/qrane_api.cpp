...
int qrane_options_check(qrane_options *opt) {
  if (opt->qrane_home == nullptr) {
    std::cout << "Please set $QRANE_HOME to your qrane installation directory "
                 "... returning error code 1\n";
    return 1;
  }
  if (opt->qasm_file == NULL) {
    std::cout
        << "You must provide an OpenQASM file ... returning error code 1\n";
    return 1;
  }
  ...
  if (opt->codegen_file) {
    if ((check = fopen(opt->codegen_file, "w"))) {
      ...
      printf("Failed to create or open codegen file %s ... returning error "
             "code 1\n",
             opt->codegen_file);
      return 1;
    }
  ...
  if (opt->aquma_file) {
    if ((check = fopen(opt->codegen_file, "w"))) {
      ...
      printf("Failed to create or open aquma output file %s .. returning error "
             "code 1\n",
             opt->aquma_file);
      return 1;
    }
...
qrane_output_scop *qrane_driver(int argc, char *argv[], qrane_options *opt) {
  ...
  if (res) {
    std::cout << "Something went wrong during reconstruction. Returning empty "
                 "scop.\n";
    return scop;
  }
  ...
  if (res) {
    std::cout << "Failed to write to aquma_file or codegen_file. Returning "
                 "empty scop.\n";
    return scop;
  }
