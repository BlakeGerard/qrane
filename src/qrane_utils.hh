...
split_into_n_components(std::vector<qrane_statement *> stmts,
                        unsigned int chunks);
int generate_c_test_file(std::string c_name, std::string qasm_name,
                         std::string codegen_c_str, std::string qreg_decls);
...
void adj_list_topological_ordering_util(
...
isl_mat *back_substitute_isl_mat(isl_ctx *ctx, __isl_keep isl_mat *mat,
                                 unsigned int args, unsigned int dim);
